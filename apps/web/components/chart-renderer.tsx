...
function BarChartComponent({ data }: { data: ChartData }) {
  ...
  const color = data.chartConfig[dataKey]?.color;
...
export function ChartRenderer({ data: chartData }: { data: ChartData }) {
  const data = {
    ...chartData,
    data: convertToNumberIfPossible(chartData.data),
  };
...
function convertToNumberIfPossible(
    ...
    for (const key in item) {
      if (item.hasOwnProperty(key)) {
        ...
        if (typeof val === "object" && val !== null) {
          newObj[key] = convertToNumberIfPossible([val])[0];
