...
const Page = () => {
  ...
  const { dbId, chatId } = useParams();

  const {
    data: dbConnection,
    isPending: isDbConnectionPending,
    isError: isDbConnectionError,
  ...
  const {
    ...
    queryFn: () => getChatWithMessages(chatId as string),
  });

  useEffect(() => {
    if (isDbConnectionError && !isDbConnectionSuccess) {
      replace("/");
    }
  }, [isDbConnectionError, isDbConnectionSuccess, replace]);

  useEffect(() => {
    if (isChatError && !isChatSuccess) {
      replace(`/`);
    }
  }, [isChatError, isChatSuccess, replace]);
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      await sendChatMessage({
        ...
        dbConnectionId: dbConnection?.data.id as string,
        chatId: (chatId as string) ?? windowChatId,
      });
    } catch (error) {
  ...
  if (isDbConnectionPending || isChatPending) {
    return (
